...
const saveOptions = () => {
	...
	const elementInfoAutoClose = document.getElementById(
		'element-info-auto-close'
	).checked
...
const restoreOptions = () => {
	chrome.storage.sync.get(
		...
		(items) => {
			...
			document.getElementById('tiny-features').checked =
				items.tinyFeatures
			document.getElementById('element-info').checked = items.elementInfo
			document.getElementById('element-info-auto-close').checked =
				items.elementInfoAutoClose
			document.getElementById('auto-published').checked =
				items.autoPublished
			document.getElementById('better-styling').checked =
				items.betterStyling
			document.getElementById('new-sorting').checked = items.newSorting
			document.getElementById('sticky-sidebar').checked =
				items.stickySidebar
...
const positionTooltip = (element, tooltip) => {
	...
	// Set tooltip class
	tooltip.className =
		tooltipClass + (tooltip.classList.contains('show') ? ' show' : '')
...
const showTooltip = (element, text) => {
	...
	const isSwitchingElements =
		currentTooltipTarget && currentTooltipTarget !== element
...
const updateTooltipPosition = () => {
	if (
		...
		tooltipElement &&
		tooltipElement.classList.contains('show')
	) {
